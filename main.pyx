...
"""
cpdef (double, int) minimax(board, int depth, bint maximizingPlayer, double alpha, double beta):
    ...
    if maximizingPlayer:
        for child in children:
            ...
            eval, evalPlace = minimax(newBoard, depth-1, False, alpha, beta)
            if eval > maxEval:
                maxEval = eval
                maxPlace = child
        ...
        for child in children:
            ...
            eval, evalPlace = minimax(newBoard, depth-1, True, alpha, beta)
            if eval < minEval:
                minEval = eval
                minPlace = child
            beta = min(beta, eval)
